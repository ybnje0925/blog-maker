...
const POLICY_EFFECTIVE_DATE = "2026-08-08";
...
function HomePage() {
  ...
  const selectedProviderLabel = "Gemini";
  const selectedApiKey = formState.geminiApiKey.trim();
  ...
  const handleSubmit = () => {
    ...
    if (!hasCustomKey && usesRemaining <= 0) {
      setErrorMessage("오늘 제공되는 무료 생성 3회를 모두 사용했습니다. 계속 이용하려면 개인 Gemini API Key를 입력해 주세요.");
      return;
    }
    ...
    trackEvent("generate_start", {
      provider: "gemini",
      api_key_mode: hasCustomKey ? "user" : "server",
      photo_count: formState.photos.length,
      has_pdf: Boolean(formState.pdfFile),
      has_reference_thumbnail: Boolean(formState.referenceThumbnailFile),
      style_level: formState.styleLevel,
    });
    performGenerate();
  };
  ...
  const handleRecommendThumbnail = async () => {
    ...
    try {
      ...
      setThumbnailData(data.thumbnailData || initialThumbnailData);
      trackEvent("thumbnail_edit", { action: "recommend_copy", provider: data.provider || "gemini" });
    } catch (err: any) {
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      trackEvent("thumbnail_edit", { action: "open_editor" });
                    }}
                    className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-bold transition ${activeTab === "thumbnail" ? "bg-blue-700 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                  >
...
const datePlaceholders = {
  publishedAt: POLICY_EFFECTIVE_DATE,
  updatedAt: POLICY_EFFECTIVE_DATE,
};
...
function GuideArticleDetailPage() {
  ...
  useEffect(() => {
    trackEvent("guide_view", { slug: article.slug, category: article.category });
      ...
      <PolicyLayout title="개인정보처리방침" description="BlogDraft 이용 과정에서 처리될 수 있는 정보와 보관 방식, 이용자의 권리를 안내합니다.">
        ...
        <PolicySection title="2. 처리할 수 있는 정보">
          ...
          <ul className="list-disc space-y-1 pl-5">
            ...
            <li>사용자가 직접 입력한 Gemini API Key</li>
          </ul>
        ...
        <PolicySection title="4. 업로드 자료 처리">
          <p>사진과 PDF는 AI 초안 생성을 위해 Google Gemini API로 전송될 수 있습니다. OpenAI 연동은 준비 중인 향후 기능입니다. 사용자는 업로드 전 타인의 개인정보, 얼굴, 주소, 연락처, 민감한 문서가 포함되어 있지 않은지 확인해야 합니다.</p>
        ...
        <PolicySection title="5. 개인 API Key 처리">
          <p>개인 Gemini API Key는 Gemini AI 요청을 처리하기 위해서만 사용됩니다. OpenAI API Key 입력과 생성 기능은 현재 제공하지 않으며, 서버 콘솔이나 로그에 API Key를 출력하지 않습니다.</p>
        ...
        <PolicySection title="7. 제3자 서비스 및 국외 처리 가능성">
          <p>현재 BlogDraft는 Google Gemini API를 사용합니다. 사용자가 입력하거나 업로드한 자료가 Google의 서버를 통해 처리될 수 있으며, 구체적인 처리 위치, 기간, 방식은 Google의 정책이 적용될 수 있으므로 공식 정책을 함께 확인해야 합니다. OpenAI 연동은 준비 중입니다.</p>
          <InfoBox title="외부 정책 확인 위치">
            <p>Google Gemini API의 개인정보 관련 공식 정책과 최신 문서를 확인해야 합니다. OpenAI 연동은 향후 기능으로 준비 중입니다.</p>
          </InfoBox>
        </PolicySection>
    ...
    <>
      ...
      <PolicyLayout title="문의하기" description="서비스 오류, 개인정보, 저작권, 제휴와 기능 개선 제안을 이메일로 문의할 수 있습니다.">
        ...
        <PolicySection title="문의 시 함께 보내면 좋은 정보">
          ...
          <InfoBox title="주의">
          <p>개인정보와 Gemini API Key는 문의 내용에 포함하지 말아 주세요.</p>
          </InfoBox>
        </PolicySection>
      </PolicyLayout>
    </>
