...
const AI_PROVIDER_OPTIONS: Array<{ id: AIProvider; title: string; body: string; disabled?: boolean }> = [
  { id: "gemini", title: "Gemini", body: "현재 운영 모델입니다. 사진과 참고자료를 활용해 블로그 초안을 생성합니다." },
  { id: "openai", title: "OpenAI", body: "준비 중입니다. 향후 추가할 기능입니다.", disabled: true },
];
...
export const InputForm: React.FC<InputFormProps> = ({ formState, setFormState, onSubmit, isGenerating, pdfBriefing, usesRemaining }) => {
  ...
  const selectedApiKey = formState.geminiApiKey;
  ...
  const updateProviderKey = (value: string) => {
    setFormState((prev) => ({ ...prev, aiProvider: "gemini", geminiApiKey: value }));
  };

  const updateRememberKey = (checked: boolean) => {
    setFormState((prev) => ({ ...prev, aiProvider: "gemini", rememberGeminiApiKey: checked }));
  };

  const deleteSelectedStoredKey = () => {
    setFormState((prev) => ({ ...prev, aiProvider: "gemini", geminiApiKey: "", rememberGeminiApiKey: false }));
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="mb-3 rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs leading-5 text-slate-700">
          오늘 무료 생성 3회가 제공됩니다. 현재는 Gemini로만 운영하며, 무료 횟수를 모두 사용한 뒤에는 개인 Gemini API Key를 연결해 계속 이용할 수 있습니다.
          OpenAI는 준비 중이며 향후 추가할 기능입니다.
        </p>
            ...
            >
              <input
                ...
                onChange={() => {
                  ...
                  setFormState((prev) => ({ ...prev, aiProvider: provider.id }));
                  trackEvent("provider_select", { provider: provider.id });
                }}
        ...
        <div className="mt-3 space-y-2">
          ...
          <label className="flex cursor-pointer items-start gap-2 text-xs leading-5 text-slate-600">
            <input
              type="checkbox"
              checked={formState.rememberGeminiApiKey}
              onChange={(event) => {
                updateRememberKey(event.target.checked);
                trackEvent("api_key_mode_select", { provider: "gemini", mode: hasSelectedApiKey ? "user" : "server", remember: event.target.checked });
              }}
      ...
      {activePanel === "photos" && (
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {formState.photos.length > 0 && (
            <div className="mt-3 grid max-h-60 grid-cols-3 gap-2 overflow-y-auto pr-1" data-clarity-mask="true">
        ...
        <section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3">
            {!formState.pdfFile ? (
              ...
              <div className="flex items-center justify-between gap-3" data-clarity-mask="true">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3">
            {!formState.referenceThumbnailPreviewUrl ? (
              ...
              <div className="flex items-center gap-3" data-clarity-mask="true">
