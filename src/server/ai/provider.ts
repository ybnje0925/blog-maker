...
import { createOpenAIProvider, OPENAI_DEFAULT_MODEL } from "./openaiProvider";
...
export function resolveProviderApiKey(provider: AIProviderName, userApiKey?: string) {
  ...
  const serverKey = provider === "openai" ? process.env.OPENAI_API_KEY : process.env.GEMINI_API_KEY;
  if (!serverKey?.trim()) {
    const label = provider === "openai" ? "OpenAI" : "Gemini";
    throw new Error(`${provider.toUpperCase()}_SERVER_KEY_MISSING: ${label} 서버 API를 사용할 수 없습니다. 계속 이용하려면 개인 ${label} API Key를 입력해 주세요.`);
  }
...
function createProvider(provider: AIProviderName, apiKey: string) {
  return provider === "openai" ? createOpenAIProvider(apiKey) : createGeminiProvider(apiKey);
}
